...
export const creditCreateRequest = async data => {
	...
	data.pres_fecha_desembolso &&
		(formatDateDesembolso = moment(data.pres_fecha_desembolso).format(
			'YYYY-MM-DD HH:mm:ss',
		));
		...
		for (const [key, value] of Object.entries(aea)) {
			...
			if (
				...
				if (Array.isArray(value)) {
					...
					const cleanNewArray = newArray.filter(element => {
	...
	try {
		...
		const adaptedCredit = {
			id: creditCreated.id,
			data: [
				creditCreated.fk_customer.first_name +
					' ' +
					creditCreated.fk_customer.last_name,
				...
				creditCreated.fk_employee_analista.first_name +
					' ' +
					creditCreated.fk_employee_analista.last_name,
				creditCreated.fk_employee_cobrador.first_name +
					' ' +
					creditCreated.fk_employee_cobrador.last_name,
			],
		};
...
export const creditUpdateRequest = async (data, id) => {
		...
		for (const [key, value] of Object.entries(aea)) {
			...
			if (
				...
				if (Array.isArray(value)) {
					...
					const cleanNewArray = newArray.filter(
	...
	const token = localStorage.getItem('token') || '';
	try {
		const { data } = await financialApi.patch(
			`/credits/credit/${id}`,
			newCrediBody,
			{
				headers: {
...
export const creditGetRequest = async id => {
	...
	try {
		...
		data.date_of_issue &&
			(formatDateEmision = moment(data.date_of_issue).format('YYYY-MM-DD'));
		data.disbursement_date &&
			(formatDateDesembolso = moment(data.disbursement_date).format(
				'YYYY-MM-DD',
			));
