...
export const PaginationContainer = ({ currentOffset, setCurrentOffset, totalPages, totalRegisters }) => {

  let currentPage = (currentOffset + 10) / 10;
  let initialPages = [currentPage + 2, currentPage + 3, currentPage + 4, currentPage + 5, currentPage + 6]
  ...
  const handleCurrentOffset = (page) => {
    ...
    let first = [3, 4, 5, 6, 7]
    let last = [totalPages - 6, totalPages - 5, totalPages - 4, totalPages - 3, totalPages - 2,]
  ...
  const paginationFirstCaseData = { currentPage, pages, setCurrentOffset, getOffset, handleCurrentOffset }
  const paginationSecondCaseData = { currentPage, handleCurrentOffset, pages }
  const paginationThidCaseData = { currentPage, handleCurrentOffset, setCurrentOffset, getOffset, pages, totalPages }
  const paginationFourthCaseData = {getOffset, setCurrentOffset, currentPage, totalPages}
  const paginationData = { totalRegisters, currentPage, handleCurrentOffset, totalPages, nextPage, prevPage }
  ...
  const PaginationButtons = () => {
    ...
    if (currentOffset >= getOffset(6) && currentOffset <= getOffset(totalPages - 5)) return (<PaginationSecondCase data={paginationSecondCaseData} />);
    if (currentOffset > getOffset(totalPages - 5)) return (<PaginationThidCase data={paginationThidCaseData} />);
  }
