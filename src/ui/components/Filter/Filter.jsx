...
export const Filter = ({ credits }) => {
  ...
  const { tablethead, dataForFilter, setDataForFilter } =
  ...
  //! FALTA TESTEAR LOS COMPONENTES DE ABAJO
  ...
  return (
    <form className="main-filter" onSubmit={handleSubmit(onSubmit)}>
      <InputsRow margin=".5" gap="1.3rem 0rem">
        ...
        <FragmentContainer
          col={rwd.buttons}
          justifyContent={`${centinela <= 1024 ? "center" : "start"}`}
        >
          <Button
            event={handleSubmit(onSearch)}
            width="8rem"
            className="primary"
            content="Buscar"
          />
          ...
          {centinela <= 1024 && (
            <Button width="4rem" className="info" content={iconFilter} />
          )}
        </FragmentContainer>
      </InputsRow>
    </form>
